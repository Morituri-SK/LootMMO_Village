--THIS WAS CHANGED, a Player was added to the callback function
--[[
	Async Blockchain
	v1.1
	by: standardcombo
...
function API.GetTokensForPlayer(player, parameters, callbackFunction)
	...
	if collectionsPerPlayer[player] then
		...
		elseif collectionsPerPlayer[player][contractAddress] then
			...
			if callbackFunction then
				--print("got callback for player",player.name)
				callbackFunction(tokens,player) --CHANGED!!!!!!
			end
			return
		end
	end
...
function Runner()
	while #requestQueue > 0 do
		...
		if callbackFunction then
			--print("got callback for player",player.name)
			callbackFunction(resultingTokens,player) --CHANGED!!!!!!
		end
	end
	activeTask = nil
end
